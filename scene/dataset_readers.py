...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png"):
    ...
    for idx, frame in tqdm(enumerate(frames)):
        ...
        c2w = np.array(frame["transform_matrix"])
        c2w[:3, 1:3] *= -1 # OpenGL (Y up, Z back) to Colmap (Y down, Z forward)
        w2c = np.linalg.inv(c2w)
        ...
        cam_infos.append(CameraInfo(uid=idx+1, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
            image_path=image_path, image_name=image_name, width=image.size[0], height=image.size[1]))
...
def readCamerasFromEV(data_path, white_background=False, device='cuda:0'):
    nerf_data = torch.load(data_path, map_location='cpu')
    dataloader = nerf_data.dataloader
    cam_infos = []
    print('Processing nerf data...')
    for ii in tqdm(range(len(dataloader))):
        
        image_path = ""
        image_name = ""
        norm_data = dataloader.images[ii].movedim(-1, 0).cpu().numpy()
        bg = np.array([1,1,1]) if white_background else np.array([0, 0, 0])
        arr = norm_data[:,:,:3] * norm_data[:, :, 3:4] + bg * (1 - norm_data[:, :, 3:4])
        image = Image.fromarray(np.array(arr*255.0, dtype=np.byte), "RGB")

        h, w = norm_data.shape[1:]
        imap = dataloader.camera.intrinsics_map[ii]
        K = dataloader.camera.intrinsics.intrinsics_matrix[imap].float()
        K[:2] /= dataloader.ds
        scale = dataloader.scene_scale_factor
        fovx = focal2fov(K[0, 0], w)
        fovy = focal2fov(K[1, 1], h)
        pose = torch.eye(4, device=device)
        pose[:3] = dataloader.camera.poses.poses[ii].float()
        pose[:3, 3] /= scale
        ...
        cam_infos.append(CameraInfo(uid=ii+1, R=R, T=T, FovY=fovy, FovX=fovx, image=image,
                        image_path=image_path, image_name=image_name, width=image.size[0], height=image.size[1]))
            
    return cam_infos

def readEVInfo(path, white_background, eval):
    print("Reading Training Transforms")
    train_cam_infos = readCamerasFromEV(path, white_background)
...
def readNerfSyntheticInfo(path, white_background, eval, extension=".png"):
    print("Reading Training Transforms")
    train_cam_infos, train_poses = readCamerasFromTransforms(path, "transforms_train.json", white_background, extension)
    print("Reading Test Transforms")
    test_cam_infos, test_poses = readCamerasFromTransforms(path, "transforms_test.json", white_background, extension)
